            ...
            <v-col cols="12" md="8" lg="11" class="mx-auto text-center mx-0 px-0 mb-lg-10 mb-md-10">
                <v-img src="../../assets/1.png" style="position: relative; display: inline-block;">
                    <v-btn class="overlay-button elevation-0" style="background-color: #734B96; border-radius: 10px;
                        position: absolute; color: white; font-weight: 800; font-size: 15px; font-family: 'My Iranian Sans', sans-serif" to="/electrical-bank/online-charge"
...
export default {
    ...
    mounted() {
        axios.get(`${this.$store.getters.getServerAddress}/transactions/total?type=DigitalCoinBox`, {headers: {'Authorization': `Bearer ${this.$store.getters.getToken}`}})
            .then(response => console.log(response))
            .catch(err => console.log(err))
        axios.get(`${this.$store.getters.getServerAddress}/transactions?type=DigitalCoinBox`, {headers: {'Authorization': `Bearer ${this.$store.getters.getToken}`}})
            .then(response => console.log(response))
            .catch(err => console.log(err))
    },
    computed: {
